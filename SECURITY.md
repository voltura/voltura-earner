...
Security reports should target the latest public Voltura Earner release or the current default branch (`main`). Older versions may receive fixes when the issue is severe and a safe patch is practical.

## Reporting a vulnerability

Do not publish exploit details in public issues.

1. Use [GitHub private vulnerability reporting](https://github.com/voltura/voltura-earner/security/advisories/new) if enabled.
...
Voltura Earner runs with the signed-in Windows user's permissions. Time and earnings calculations happen locally. Stored settings and work records are bounded and validated before use. Settings and diagnostic files are not a security boundary against other software running as the same user.
